...
int getMax(Jogador *jogadores, int n) {
    int mx = jogadores[0].id;
    for (int i = 1; i < n; i++)
        if (jogadores[i].id > mx)
            mx = jogadores[i].id;
    return mx;
}

void countSort(Jogador *jogadores, int n, int exp) {
    Jogador output[n];
    int i, count[10] = {0};
...
void radixsort(Jogador *jogadores, int n) {
    int m = getMax(jogadores, n);

    for (int exp = 1; m / exp > 0; exp *= 10)
        countSort(jogadores, n, exp);
}
